...
# Configuration
output_path = "/home/visione/Projects/BlenderScenarios/Sat/Dataset/Orbit_VR_256_dynlight/VIS_Test"
image_format = 'PNG'
resolution = (256, 256)
num_frames = 15
camera_distance = 12
json_output_path = os.path.join(output_path, "transforms.json")
cam_flag = ['V', 'R']
...
satellite = bpy.data.objects['Layer_0'] 
sun = bpy.data.objects["Light"]

# Set Object Index for Masking
satellite.pass_index = 1
...
for n_orbit, orbit in enumerate(cam_flag):
    for frame in range(num_frames):
        ...
        cam2obj = obj2world @ (vcam2obj if orbit == 'V' else rcam2obj @ rcam2rcam)

        camera.location = cam2obj.translation
        camera.rotation_quaternion = cam2obj.to_quaternion()

        sun.rotation_quaternion = mathutils.Quaternion((random.uniform(-1, 1), random.uniform(-1, 1), random.uniform(-1, 1), random.uniform(-1, 1))).normalized()
        sun2world = sun.rotation_quaternion.to_matrix().to_4x4()
